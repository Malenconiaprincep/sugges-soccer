import axios from "axios"
import { imgs } from "../notionpic"
import "./index.css"

// const host = `http://10.255.128.9:9999`
const host = `http://localhost:9999`

const App = () => {
  const data = {
    notionLogo: "./data/korea/teams-974-180.png",
    notionName: "韩国",
    players: [
      {
        left: "0.8%",
        top: "6%",
        img: "./data/korea/players-200-104-22_180.png",
        href: "/player/200104/heung-min-son/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Son",
        teamlogo: "./data/korea/teams-18-180.png",
        pac: 88,
        sho: 88,
        pas: 82,
        dri: 86,
        def: 42,
        phy: 69,
        rating: "89",
        pos: "LW",
      },
      {
        left: "20.16%",
        top: "-2.5%",
        img: "./data/korea/players-226-380-22_180.png",
        href: "/player/226380/hee-chan-hwang/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Hwang Hee Chan",
        teamlogo: "./data/korea/teams-110-180.png",
        pac: 83,
        sho: 75,
        pas: 70,
        dri: 78,
        def: 28,
        phy: 70,
        rating: "77",
        pos: "LW",
      },
      {
        left: "51.84%",
        top: "-2.5%",
        img: "./data/korea/players-212-260-22_180.png",
        href: "/player/212260/ui-jo-hwang/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Hwang Ui Jo",
        teamlogo: "./data/korea/teams-59-180.png",
        pac: 74,
        sho: 78,
        pas: 65,
        dri: 74,
        def: 42,
        phy: 71,
        rating: "75",
        pos: "ST",
      },
      {
        left: "71.2%",
        top: "6%",
        img: "./data/korea/players-243-780-22_180.png",
        href: "/player/243780/kang-in-lee/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Lee Kang In",
        teamlogo: "./data/korea/teams-453-180.png",
        pac: 72,
        sho: 73,
        pas: 74,
        dri: 77,
        def: 30,
        phy: 54,
        rating: "74",
        pos: "CAM",
      },
      {
        left: "20.16%",
        top: "33.75%",
        img: "./data/korea/players-221-671-22_180.png",
        href: "/player/221671/jae-sung-lee/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Lee Jae Sung",
        teamlogo: "./data/korea/teams-169-180.png",
        pac: 66,
        sho: 62,
        pas: 75,
        dri: 80,
        def: 57,
        phy: 62,
        rating: "76",
        pos: "CM",
      },
      {
        left: "51.84%",
        top: "33.75%",
        img: "./data/korea/players-180-283-22_180.png",
        href: "/player/180283/sung-yueng-ki/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Ki Sung Yueng",
        teamlogo: "./data/korea/teams-982-180.png",
        pac: 46,
        sho: 68,
        pas: 77,
        dri: 68,
        def: 64,
        phy: 75,
        rating: "74",
        pos: "CM",
      },
      {
        left: "-3.16%",
        top: "62.5%",
        img: "./data/korea/players-222-734-22_180.png",
        href: "/player/222734/sang-woo-kang/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Kang Sang Woo",
        teamlogo: "./data/korea/teams-111768-180.png",
        pac: 81,
        sho: 63,
        pas: 69,
        dri: 68,
        def: 64,
        phy: 62,
        rating: "71",
        pos: "LB",
      },
      {
        left: "20.16%",
        top: "70%",
        img: "./data/korea/players-211-005-22_180.png",
        href: "/player/211005/hyeon-soo-jang/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Jang Hyeon Soo",
        teamlogo: "./data/korea/teams-605-180.png",
        pac: 64,
        sho: 32,
        pas: 59,
        dri: 63,
        def: 76,
        phy: 75,
        rating: "74",
        pos: "CB",
      },
      {
        left: "51.84%",
        top: "70%",
        img: "./data/korea/players-237-086-22_180.png",
        href: "/player/237086/min-jae-kim/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Kim Min Jae",
        teamlogo: "./data/korea/teams-48-180.png",
        pac: 80,
        sho: 33,
        pas: 57,
        dri: 62,
        def: 77,
        phy: 82,
        rating: "77",
        pos: "CB",
      },
      {
        left: "75.16%",
        top: "62.5%",
        img: "./data/korea/players-199-231-22_180.png",
        href: "/player/199231/tae-hwan-kim/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Kim Tae Hwan",
        teamlogo: "./data/korea/teams-1473-180.png",
        pac: 88,
        sho: 57,
        pas: 61,
        dri: 69,
        def: 65,
        phy: 80,
        rating: "72",
        pos: "RB",
      },
      {
        left: "36%",
        top: "83.5%",
        img: "./data/korea/players-212-432-22_180.png",
        href: "/player/212432/hyeon-woo-jo/",
        defaultimg: "https://cdn.sofifa.net/players/notfound_0_60.png",
        name: "Jo Hyeon Woo",
        teamlogo: "./data/korea/teams-1473-180.png",
        pac: 75,
        sho: 71,
        pas: 62,
        dri: 80,
        def: 51,
        phy: 71,
        rating: "74",
        pos: "GK",
      },
    ],
    teamname: "korea",
  }

  const List: any = data.players.map((item: any, index) => {
    let classCard = ""

    if (item.rating >= 80) {
      classCard = "card-22-create-gold"
    } else if (item.rating >= 70) {
      classCard = "card-22-create-silver"
    } else {
      classCard = "card-22-create-bronze"
    }

    return (
      <div>
        <div
          id={`card${index}`}
          className={`card-22-create ${classCard}`}
          key={index}
        >
          <div className="card-22-create-face">
            <div className="card-22-create-face-inner">
              <img
                src={item.img}
                alt="Lionel Messi FIFA 18 Custom Card Creator Face"
              />
            </div>
          </div>
          <div className="card-22-create-badge2">
            <img src={data.notionLogo} width="100" height="100" />
          </div>
          {/* <div className="card-22-create-flag">
            <img src={data.notionLogo} width="72" height="43" />
          </div> */}
          <div className="card-22-create-name">{item.name}</div>
          <div className="card-22-create-rating">{item.rating}</div>
          <div className="card-22-create-position">{item.pos}</div>
          <div className="card-22-create-atts">
            <div className="card-22-create-atts1">
              <div className="card-22-create-attnum card-22-create-attnum1">
                {item.pac}
              </div>
              <div className="card-22-create-attlabel card-22-create-attlabel1">
                PAC
              </div>
              <div className="card-22-create-attnum card-22-create-attnum4">
                {item.dri}
              </div>
              <div className="card-22-create-attlabel card-22-create-attlabel4">
                DRI
              </div>
            </div>
            <div className="card-22-create-atts2">
              <div className="card-22-create-attnum card-22-create-attnum2">
                {item.sho}
              </div>
              <div className="card-22-create-attlabel card-22-create-attlabel2">
                SHO
              </div>
              <div className="card-22-create-attnum card-22-create-attnum5">
                {item.def}
              </div>
              <div className="card-22-create-attlabel card-22-create-attlabel5">
                DEF
              </div>
            </div>
            <div className="card-22-create-atts3">
              <div className="card-22-create-attnum card-22-create-attnum3">
                {item.pas}
              </div>
              <div className="card-22-create-attlabel card-22-create-attlabel3">
                PAS
              </div>
              <div className="card-22-create-attnum card-22-create-attnum6">
                {item.phy}
              </div>
              <div className="card-22-create-attlabel card-22-create-attlabel6">
                PHY
              </div>
            </div>
          </div>
          <div className="card-22-create-headeroverlay"></div>
          {/* <div className="card-22-create-split card-22-create-flag-split"></div> */}
          {/* <div className="card-22-create-split card-22-create-name-split"></div> */}
          <div className="card-22-create-split card-22-create-atts-split"></div>
          <div className="card-22-create-split card-22-create-chem-split"></div>
        </div>
        <button
          onClick={() => {
            const cardid = document.querySelector(
              `#card${index}`
            ) as HTMLElement
            ;(window as any)
              .html2canvas(cardid, {
                backgroundColor: null,
                width: 450,
                height: 650,
                allowTaint: true,
                useCORS: true,
              })
              .then((canvas: any) => {
                document.body.appendChild(canvas)
              })
              .catch((err: any) => {
                alert(err)
              })
          }}
        >
          打印
        </button>
      </div>
    )
  })

  return <div>{List}</div>
}

export default App
